...
class Gen_Model():
	...
	def write(self, game, version):
		fileName = run_folder + 'models/version' + "{0:0>4}".format(version) + '.h5'
		self.model.save(fileName)

	def read(self, game, run_number, version):
		fileName = run_archive_folder + game + '/run' + str(run_number).zfill(4) + "/models/version" + "{0:0>4}".format(version) + '.h5'
        #run_archive_folder + game + '/run' + str(run_number).zfill(4) + "/models/version" + "{0:0>4}".format(version) + '.h5'
		return load_model(fileName, custom_objects={'softmax_cross_entropy_with_logits': softmax_cross_entropy_with_logits})
